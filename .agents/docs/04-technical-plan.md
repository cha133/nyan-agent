...
- **进程树清理**：shell 工具仍用 Windows `taskkill /T /F` 终止单次命令树；Rust supervisor 额外用 `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` 的独立 Job Object 拥有整个 Bun backend generation，覆盖 Bun 自身 crash 时无法再按父 PID 执行 `taskkill` 的窗口。真实孙进程延迟写测试、backend crash 桌面 E2E 和安装版 app 退出 smoke 均确认没有遗留。
