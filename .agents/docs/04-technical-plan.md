...
命令带 `requestId`，turn 流带 `sessionId` 和 `turnId`，tool 事件再带 `toolCallId`。第一批消息建议包括：
...
- 项目列表、无项目任务列表、每个项目的任务列表各自维护独立展开状态：默认只取前 5 项；超过 5 项显示“展开显示”，展开后显示全部并切换为“折叠显示”。切换不改变持久化排序。
...
9. HeroUI/Lexical 页面、静态 Markdown block、5 条折叠、Mica spike、交互打磨。
...
1. provider 为 Anthropic-compatible 与 OpenAI-compatible，凭据写 `~/.config/nyan/config.toml`。
...
- **Mica + 原生标题栏**：尽早用真实 Win11 目标版本验证透明、inactive、resize 和深浅色，不以浏览器预览代替。
...
- **配置文件含明文密钥**：MVP 接受该取舍，但创建目录/文件时限制为当前用户可访问，并且日志、错误与 UI 永不回显完整 key。
- **PowerShell 编码与命令长度**：`-EncodedCommand` 解决转义和 Unicode，但 Base64 会放大命令；需要对 Windows 命令行长度做实测，超长脚本可回退到受控临时 `.ps1` 文件。
- **进程树清理**：只 kill `pwsh` 父进程可能遗留子进程；必须做 Windows Job Object 或等价的进程树方案 spike。
