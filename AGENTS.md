...
- 一次调用只接受一个任务；需要并行时，主模型在同一响应中提交多个 subagent tool call，由 AI SDK 并行执行，不设置应用层 subagent 数量上限。
- subagent 使用同一模型与独立上下文，只能使用共享的 shell/edit，不能递归调用 subagent。
- 委派提示必须写清范围、预期结果、是否允许修改；不要让多个并行写任务修改同一文件。
- 每个调用独立返回结果；单项失败不能丢失并行调用的成功结果。父 AbortSignal 必须级联到子模型和 shell 进程。
