...
- 自绘标题栏 `decorations: false`（高 36px 独立顶栏，右侧窗口控件；brand 在侧栏顶部）；窗口启用透明 WebView 与 Mica，侧栏透出材质，主内容在标题栏下方起排并保留向左投影（可渗入标题栏条带）。最大化按钮经 HTMAXBUTTON overlay 保留 Win11 Snap Layout；caption 图标使用 Segoe Fluent Icons。
