...
- 自绘标题栏高度 36px（独立顶栏，右侧主面板在其下方起排，仿 Codex）；左侧放 brand logo，右侧放仿 Win11 最小化/最大化/关闭按钮（最大化保留系统 Snap Layout 悬停；图标用 Segoe Fluent Icons）；不做“文件/视图”等菜单。
- 标题栏与左侧栏透出 Win11 Mica；主体保持不透明白色层级，向左投影可向上渗入标题栏条带。
